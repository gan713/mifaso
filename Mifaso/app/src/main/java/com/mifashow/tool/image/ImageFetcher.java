...
public class ImageFetcher extends ImageResizer {
    ...
    private static final int HTTP_CACHE_SIZE = 100 * 1024 * 1024; // 50MB
    public static final String HTTP_CACHE_DIR = "bitmaps";
...
//            Toast.makeText(context, "No network connection found.", Toast.LENGTH_LONG).show();
    ...
    public static File downloadBitmap(Context context, String urlString) {
        ...
        if (cache.containsKey(urlString)) {
        	Log.d("-ImageFetcher:cache:",urlString);
            return cacheFile;
        }else{
        	Log.d("-ImageFetcher:nocache",urlString);
        }
...
//            }
    ...
    public static File fetchBitmap(Context context, int resId) {
        final File cacheDir = DiskLruCache.getDiskCacheDir(context, HTTP_CACHE_DIR);

        final DiskLruCache cache =
                DiskLruCache.openCache(context, cacheDir, HTTP_CACHE_SIZE);

        final File cacheFile = new File(cache.createFilePath(context.getResources().getString(resId)));

        if (cache.containsKey(context.getResources().getString(resId))) {
            return cacheFile;
        }
        BufferedOutputStream out = null;
        ...
        try {
            ...
            final InputStream in =
                    new BufferedInputStream(context.getResources().openRawResource(resId), Utils.IO_BUFFER_SIZE);
            out = new BufferedOutputStream(new FileOutputStream(cacheFile), Utils.IO_BUFFER_SIZE);
